...
def write4cif(name,chg,charge = False):
    if charge:
        ...
        new_content[0] = "# generated by Guobin Zhao (Pusan National University)\n"
        new_content[1] = "data_structure\n"
        for i, line in enumerate(new_content):
            if '_atom_site_occupancy' in line:
                new_content.insert(i + 1, "  _atom_site_charge\n")
                break
        charge_index = 0
        for j in range(i + 2, len(new_content)):
            if charge_index < len(charges):
                new_content[j] = lines[j].strip() + " " + str(charges[charge_index]) + "\n"
                charge_index += 1
            else:
                break
    return new_content
