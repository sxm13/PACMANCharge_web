...
# from cell, position of each atom, DDEC charge of each atom to cif file
def write_cif(data_csv,cell_dir,pos_dir,ddec_dir,save_dir,ddec = False):
    mofs = pd.read_csv(data_csv)["name"]
    for mof in mofs:
        cell_data = np.load(cell_dir + mof + ".npy")
        atoms = Atoms(cell=cell_data)
        a, b, c, alpha, beta, gamma = atoms.get_cell_lengths_and_angles()
        pos_data = np.load(pos_dir + mof + ".npy")
        ele = []
        xyz = []
        for i in range(pos_data.shape[0]):
            ele.append(periodic_table_symbols[int(pos_data[i][0] - 1)])
            xyz.append([pos_data[i][1],pos_data[i][2],pos_data[i][3]]) 
        lattice = Lattice.from_parameters(a=a, b=b, c=c, alpha=alpha, beta=beta, gamma=gamma)
        structure = Structure(lattice, ele, xyz)
        cif_writer = CifWriter(structure)
        cif_writer.write_file(save_dir + mof + '.cif')
        if ddec:
            ddec_charge = np.load(ddec_dir + mof + ".npy")
            new_cif = ""
            i = 0
            prop_lines = False
            with open(save_dir + mof + '.cif', "r") as f:
                for line in f:
                    ...
                    if prop_lines:
                        new_cif += line.strip()
                        new_cif += f"  {ddec_charge[i]}"
                        new_cif += "\n"
                        i += 1
                    else:
                        new_cif += line
            with open(save_dir + mof + '.cif', "w") as f:
                f.write(new_cif)
        return mof,ddec
